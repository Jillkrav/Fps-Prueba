...
## Categoría del arma (Pistolas, Rifles, Subfusiles, etc). Se carga desde skill.json.
var categoria_arma: String = ""
## Multiplicador de dispersión al apuntar (ADS) por categoría de arma.
## 1.0 = sin reducción, 0.1 = 90% más precisa.
const ADS_SPREAD_BY_CATEGORY: Dictionary = {
	"Pistolas":        0.8,  # 20% más precisa
	"Subfusiles":      0.6,  # 40% más precisa
	"Rifles":          0.4,  # 60% más precisa
	"Escopetas":       0.5,  # 50% más precisa
	"Francotiradores": 0.1,  # 90% más precisa
	"Explosivas":      0.5,  # Bazooka/lanzagranadas
	"Arrojadizas":     0.5,
	"Plasma":          0.5,
}
...
# ─── Escena de proyectil ────────────────────────────────────────────────
var projectile_scene: PackedScene = preload("res://scenes/Compartido/projectiles/projectile_placeholder.tscn")

# ─── Escena de estela de bala/perdigón ───────────────────────────────────
var bullet_trail_scene: PackedScene = preload("res://scenes/Compartido/effects/bullet_trail.tscn")
...
## Bloqueo de equipamiento (Fase 3). Cuando está true, el arma NO puede
## disparar ni recargar, independientemente de quién llame a fire().
## Lo gestiona WeaponEquipState a través de set_equip_locked().
var _equip_locked: bool = true
...
func initialize_from_name(nombre_arma: String) -> void:
	...
	categoria_arma      = ConfigManager.get_categoria_arma(nombre_arma)
...
## Establece si el arma está bloqueada por equipamiento.
## Llamado por WeaponEquipState cuando se equipa/guarda el arma.
func set_equip_locked(locked: bool) -> void:
	_equip_locked = locked


## ¿Se puede disparar? Ahora también verifica el bloqueo de equipamiento.
func can_fire() -> bool:
	if _equip_locked:
		return false
	if is_reloading:
...
## Si la categoría no está en el diccionario, usa 0.5 por defecto.
func _get_ads_multiplier_by_category() -> float:
	return ADS_SPREAD_BY_CATEGORY.get(categoria_arma, 0.5)

## Retorna el spread efectivo según el estado ADS y la categoría del arma.
func _get_effective_spread() -> float:
	if is_ads:
		var mult: float = _get_ads_multiplier_by_category()
		if mult < 1.0:
			return spread * mult
	return spread
...
func perform_raycast_with_spread() -> void:
	...
	var effective_spread: float = _get_effective_spread()
	...
	if _hitscan_target_override != Vector3.ZERO:
		...
		if effective_spread > 0.0:
			...
			var spread_offset: float = effective_spread * dist_to_target
...
func _fire_projectile() -> Array:
	...
	var projectile_spread: float = _get_effective_spread()
	if projectile_spread > 0.0:
		var rx: float = randf_range(-projectile_spread, projectile_spread)
		var ry: float = randf_range(-projectile_spread, projectile_spread)
...
func _apply_melee_knockback(target: Node3D, attacker: Node3D) -> void:
	...
	if target is CharacterBody3D:
		# Para CharacterBody3D (Player, BotBase): aplicar directamente a la velocidad
...
func load_ai_profile() -> bool:
	...
	var path: String = "res://config/Multiplayer/ai_profiles/" + filename + ".tres"
