...
const CONFIG_PATH := "res://config/Compartido/skill.json"
...
func _load_config() -> void:
	if not FileAccess.file_exists(CONFIG_PATH):
		push_error("ConfigManager: No se encontro skill.json en " + CONFIG_PATH)
		return
	...
	print("ConfigManager: skill.json cargado OK. Claves: ", _data.keys())
...
## Devuelve la categoría a la que pertenece un arma (Pistolas, Rifles, etc).
## Retorna "" si no se encuentra.
func get_categoria_arma(nombre: String) -> String:
	var armas: Dictionary = _data.get("Armas", {})
	for cat_key: String in armas.keys():
		if armas[cat_key].has(nombre):
			return cat_key
	return ""
