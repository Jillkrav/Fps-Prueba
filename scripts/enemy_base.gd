...
func _init_weapon(nombre: String) -> void:
	...
	if _weapon_cfg.is_empty():
		push_warning("EnemyBase: arma '%s' no encontrada en ConfigManager, usando defaults." % nombre)
		return
...
func _npc_fire_weapon() -> void:
	if not target_player or not target_player.has_method("take_damage"):
		return
	var damage_to_apply: float = float(_weapon_cfg.get("DañoAlJugador", 10.0))
	target_player.take_damage(damage_to_apply)
...
func perform_attack() -> void:
	if not _weapon_cfg.is_empty():
		...
		target_player.take_damage(10.0)
