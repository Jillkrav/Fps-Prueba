...
# max_health se carga desde ConfigManager en _ready()
...
func setup_weapon() -> void:
	...
	# Leer arma seleccionada desde GameState; default USP
	...
	if active_weapon.can_fire():
		...
		for hit in hits:
			...
			if target and target.has_method("take_damage"):
				if target is Player:
					target.take_damage(hit["damage_vs_player"])
				else:
					target.take_damage(hit["damage_vs_npc"])
