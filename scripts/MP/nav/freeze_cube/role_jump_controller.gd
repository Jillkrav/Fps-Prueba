# scripts/role_jump_controller.gd
# ──────────────────────────────────────────────────────────────────
# Controlador de salto por rol — se añade como hijo del NPC cuando
# el FreezeCube0 lo congela.
#
# Maneja el movimiento parabólico hacia el SemanticPoint del rol
# correspondiente (ASSAULT o ALTERNATE) y luego descongela al NPC
# automáticamente.
# ──────────────────────────────────────────────────────────────────
extends Node
class_name RoleJumpController
...
var _freeze_duration: float = 1.5
var _elapsed: float = 0.0

## Si no hay punto semántico disponible, solo congelar sin salto.
...
func setup(
	...
	_jump_target.y += 0.5  # Aterrizar sobre el punto
...
func _finish(npc: NpcBase) -> void:
	if npc and is_instance_valid(npc):
		...
		# ── Resetear ruta del StateRoaming ─────────────────────
		# Tras un salto a otro piso, _nav_target apunta a la ruta
		# del piso anterior. Lo reseteamos para que el roaming
		# recalcule una nueva ruta desde la posición actual.
		if npc.decision_sys:
			var roaming := npc.decision_sys.get_node_or_null("State_Roaming")
			if roaming:
				roaming._nav_target = Vector3.ZERO
				roaming._nav_target_set_time = 0.0
				roaming._semantic_path.clear()
