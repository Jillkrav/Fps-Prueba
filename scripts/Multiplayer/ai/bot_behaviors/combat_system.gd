...
#   weapon_status (WeaponSystem / BotBase._weapon)
#
# ── NUNCA ESCRIBE ──
...
## ¿El bot está usando ADS (apuntado preciso)? Fase 5 — lo decide
## según el perfil del arma: si range_rating < 0.5, apunta.
var is_using_ads: bool = false
...
func process(_delta: float) -> void:
	...
	# ── 4. Decidir si usar ADS según perfil del arma (Fase 5) ──
	var was_ads: bool = is_using_ads
	is_using_ads = _should_use_ads()
	if is_using_ads != was_ads:
		_debug("ADS %s (range_rating<0.5)" % ["ON" if is_using_ads else "OFF"])

	# ── 5. Trackear daño recibido para dodge reactivo ──
	_track_damage()
	
	# ── 6. Actualizar engagement_analysis ──
...
## Retorna true si range_rating < 0.5 → el arma está fuera de su rango óptimo,
## por lo que el bot debe apuntar con más cuidado.
## Armas cuerpo a cuerpo nunca usan ADS.
func _should_use_ads() -> bool:
	var weapon: Weapon = _get_weapon()
	if not weapon:
		return false
	# Cargar perfil si no está cargado
	var profile: WeaponAIProfile = weapon.ai_profile
	if not profile:
		if not weapon.load_ai_profile():
			return false
		profile = weapon.ai_profile
	if not profile:
		return false
	# Cuerpo a cuerpo nunca usa ADS
	if profile.is_melee:
		return false
	# Evaluar distancia
	var dist: float = decision_sys.dist_to_target() if decision_sys else 15.0
	var rating: float = profile.range_rating(dist)
	# Si el arma está fuera de rango óptimo, apuntar cuidadosamente
	return rating < 0.5
...
func _check_fire_weapon(cmd: CombatCommand) -> void:
	...
	if not weapon.can_fire():
		return
	# No disparar durante cooldown de equipamiento (animación) (Fase 4)
	if bot and bot.weapon_equip_state and not bot.weapon_equip_state.can_shoot():
		return

	# ── VERIFICAR LÍNEA DE VISIÓN: no disparar a través de paredes ──
	# Si el objetivo está detrás de un obstáculo, el bot NO debe disparar.
	# Esto previene que los bots "wallhackeen" a los enemigos escondidos.
	# Esta verificación corre SIEMPRE, incluso con force_fire=true,
	# porque disparar a través de paredes rompe la inmersión.
	if decision_sys and decision_sys.has_target():
		if not _has_line_of_sight_to_target():
			return  # Sin línea de visión: no disparar
...
func _is_aiming_at_target() -> bool:
	...
	var weapon: Weapon = _get_weapon()
	if weapon:
		match weapon.categoria_municion:
			"arrojadiza", "explosiva", "plasma":
				acceptance_angle = 25.0  # Más tolerante para proyectiles
	# Si el bot está usando ADS (Fase 5), requiere más precisión antes de disparar
	if is_using_ads:
		acceptance_angle *= 0.5  # Mitad de tolerancia: 7.5° para hitscan, 12.5° para proyectiles
...
## Verifica si hay línea de visión directa hacia el objetivo actual.
## Retorna true si el rayo desde la cabeza del bot impacta en el objetivo
## (o en algún hijo suyo), false si impacta en otra cosa (pared, obstáculo).
## Esto evita que los bots disparen a través de paredes.
func _has_line_of_sight_to_target() -> bool:
	...
	if target is CharacterBody3D:
		target_pos += Vector3.UP * 0.9  # Centro del torso
	else:
		target_pos += Vector3.UP * 0.7
	
	# Si está demasiado cerca, no necesita raycast
	if origin.distance_squared_to(target_pos) < 1.0:
		return true
	
	# Lanzar raycast desde la cabeza hasta el objetivo
	var space_state: PhysicsDirectSpaceState3D = bot.get_world_3d().direct_space_state
	if space_state == null:
		return true  # No podemos verificar, permitir disparo por seguridad
	
	var query := PhysicsRayQueryParameters3D.create(origin, target_pos)
	# Usar la misma máscara de colisión que el raycast de visión del bot
	query.collision_mask = bot.raycast_vision.collision_mask if is_instance_valid(bot.raycast_vision) else 1
	# Excluir al bot mismo del raycast
	# Nota: exclude acepta Array[RID] pero Godot convierte automáticamente
	# objetos con get_rid() (como CharacterBody3D) a RID.
	query.exclude = [bot]
	
	var result: Dictionary = space_state.intersect_ray(query)
	if result.is_empty():
		# No impactó nada — raro pero posible si no hay geometría
		return true
	...
	# Verificar si el collider es el objetivo o un hijo del objetivo
	return _is_collider_target_or_child(collider, target)


## Verifica recursivamente si un collider es el objetivo o un nodo hijo del objetivo.
func _is_collider_target_or_child(collider: Node, target: Node3D) -> bool:
	if collider == target:
		return true
...
## Equivalente a _aim_at_target() en BotBase.
