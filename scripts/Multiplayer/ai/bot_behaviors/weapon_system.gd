...
const AI_PROFILES_DIR: String = "res://config/Multiplayer/ai_profiles/"
...
## Rol del bot (Roles.Type). WeaponSystem lo usa para aplicar
## preferencias de armas basadas en el rol (vía RoleWeapons).
var role_type: int = Roles.Type.ASALTO
...
func _ready() -> void:
	...
	# Leer el rol del bot automáticamente
	if bot:
		role_type = bot.rol


## Procesa el estado del arma cada frame.
## Llamar desde BotBase._physics_process().
func process(delta: float) -> void:
	...
	if _switch_check_timer >= 2.0 and available_weapons.size() > 1:
...
func _get_weapon_rating(weapon: Weapon, context: Dictionary = {}) -> float:
	...
	var rating: float = profile.evaluate(eval_context)

	# ── FASE 2: Multiplicador por rol ────────────────────────────
	# Si el arma coincide con las preferencias del rol del bot,
	# se aplica un bonus al rating para que sea más probable que
	# el bot la elija sobre otras.
	if profile.category != "" and RoleWeapons.is_weapon_preferred(role_type, weapon.weapon_name, profile.category):
		var bonus: float = RoleWeapons.get_category_bonus(role_type)
		rating *= bonus
