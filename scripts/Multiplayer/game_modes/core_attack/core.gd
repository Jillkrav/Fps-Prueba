...
func _register_in_gamestate() -> void:
	if is_instance_valid(GameState):
		GameStateMP.register_core(self)

func _on_health_changed(current: float, max_val: float) -> void:
	if is_instance_valid(GameState):
		GameStateMP.on_core_health_updated(team, current, max_val)
	_update_health_label(current, max_val)

func _on_core_destroyed(_team: int) -> void:
	if is_instance_valid(GameState):
		GameStateMP.on_core_destroyed(_team)
...
func take_damage(amount: float, _zone: String = "Torso", _killer_id: int = -1, _from_position: Vector3 = Vector3.INF) -> void:
	if is_destroyed:
		return
