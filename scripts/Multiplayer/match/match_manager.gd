...
# NOTA: usar GameStateMP.match_ended en lugar de esta
...
@export var max_players_total: int = 100:
	set(val):
...
## Prefab del bot NPC
@export var bot_scene: PackedScene = preload("res://scenes/Multiplayer/objetos/bots/bot.tscn")
...
func set_bot_skin_ids(blue_id: String, red_id: String) -> void:
	_blue_bot_skin_id = blue_id
	_red_bot_skin_id = red_id

var bot_pool: Array[BotBase] = []
...
var _inactive_bots: Array[BotBase] = []
...
## Configuracion de distribucion de roles (cargada desde JSON).
var _role_distribution_config: Dictionary = {}
...
## Busqueda inversa: pawn (Player o BotBase) -> player_id
...
func _ready() -> void:
	# Tomar el maximo de jugadores desde GameState (configurado desde el menu)
	if is_instance_valid(GameState) and GameStateMP.max_players_total > 0:
		max_players_total = GameStateMP.max_players_total
	_actualizar_limites()
	_load_role_distribution()
...
## Carga la configuracion de distribucion de roles desde JSON.
func _load_role_distribution() -> void:
	var file: FileAccess = FileAccess.open("res://config/Multiplayer/role_distribution.json", FileAccess.READ)
	if file == null:
		push_error("[MatchManager] No se pudo abrir role_distribution.json")
		_role_distribution_config = {
			"base_roles": ["VERSATIL", "ASALTO", "FLANQUEADOR", "DEFENSOR", "PATRULLADOR"],
			"specialist_roles": ["FRANCOTIRADOR", "APOYO"],
			"specialist_frequency": 0.15,
		}
		return

	var json_str: String = file.get_as_text()
	var json_parser: JSON = JSON.new()
	var parse_err: Error = json_parser.parse(json_str)
	if parse_err != OK:
		push_error("[MatchManager] Error parseando role_distribution.json: %s" % json_parser.get_error_message())
		return

	_role_distribution_config = json_parser.data
	# ceil() maneja pares e impares: 24→12, 25→13
	max_per_team = ceil(max_players_total / 2.0)
...
func _crear_pool_de_bots(cantidad: int) -> void:
	...
	for i in range(cantidad):
		var bot: BotBase = bot_scene.instantiate() as BotBase
		bot_pool.append(bot)
...
func _asignar_bots_a_equipos(player_team_id: int, count_player_team: int, count_other_team: int) -> void:
	...
	for bot in bot_pool:
		...
		bot.skin_id = _blue_bot_skin_id if team == int(Enums.Equipo.AZUL) else _red_bot_skin_id
		if bot.has_method("set_runtime_skin"):
			bot.call_deferred("set_runtime_skin", bot.skin_id)
...
## Genera un array de roles distribuidos segun role_distribution.json.
## Los roles base se reparten equitativamente.
## Los especialistas aparecen con frecuencia reducida.
func _generar_roles_para_equipo(cantidad: int) -> Array[int]:
	var base_role_names: Array = _role_distribution_config.get("base_roles", [])
	var specialist_names: Array = _role_distribution_config.get("specialist_roles", [])
	var specialist_freq: float = _role_distribution_config.get("specialist_frequency", 0.15)

	# Convertir nombres a int segun el enum Roles.Type
	var role_name_to_int: Dictionary = {
		"VERSATIL": int(Roles.Type.VERSATIL),
		"ASALTO": int(Roles.Type.ASALTO),
		"FLANQUEADOR": int(Roles.Type.FLANQUEADOR),
		"DEFENSOR": int(Roles.Type.DEFENSOR),
		"PATRULLADOR": int(Roles.Type.PATRULLADOR),
		"FRANCOTIRADOR": int(Roles.Type.FRANCOTIRADOR),
		"APOYO": int(Roles.Type.APOYO),
	}

	var base_roles: Array[int] = []
	for role_name_str in base_role_names:
		var val: Variant = role_name_to_int.get(role_name_str, null)
		if val != null:
			base_roles.append(val)

	var specialist_roles: Array[int] = []
	for role_name_str in specialist_names:
		var val: Variant = role_name_to_int.get(role_name_str, null)
		if val != null:
			specialist_roles.append(val)

	if base_roles.is_empty():
		base_roles = [int(Roles.Type.ASALTO)]

	var resultado: Array[int] = []

	# Decidir cuantos especialistas poner
	var specialist_count: int = floori(cantidad * specialist_freq)
	var base_count: int = cantidad - specialist_count

	# Distribuir base roles equitativamente
	if base_count > 0:
		var base_valores: Array[int] = []
		var base_n: int = base_roles.size()
		var b: int = floori(base_count / float(base_n))
		var e: int = base_count % base_n
		for i in range(base_n):
			var veces: int = b
			if i < e:
				veces += 1
			for _j in range(veces):
				base_valores.append(base_roles[i])
		resultado.append_array(base_valores)

	# Agregar especialistas
	for _k in range(specialist_count):
		if not specialist_roles.is_empty():
			resultado.append(specialist_roles[randi() % specialist_roles.size()])
		else:
			resultado.append(base_roles[randi() % base_roles.size()])

	resultado.shuffle()
	return resultado

func _activar_bot(bot: BotBase, team: int) -> void:
	...
	# Asignar arma según el rol del bot
	var armas: Array[String] = ConfigManager.get_nombres_armas()
	if not armas.is_empty():
		bot.nombre_arma = RoleWeapons.pick_weapon_for_role(bot.rol, armas)
...
func reportar_muerte_bot(bot: BotBase) -> void:
...
func _process(delta: float) -> void:
	...
	for pawn in pawns_a_respawn:
		...
		if pawn == player:
			...
			_respawnear_bot(pawn as BotBase)
...
func _respawnear_bot(bot: BotBase) -> void:
	...
	# ── Asignar arma según el rol del bot antes de reaparecer ──
	# Esto asegura que no conserve el arma que tenia antes de morir.
	var armas: Array[String] = ConfigManager.get_nombres_armas()
	if not armas.is_empty():
		bot.nombre_arma = RoleWeapons.pick_weapon_for_role(bot.rol, armas)
	
	# Delegar la restauracion interna al propio bot (respawn() en bot_base.gd)
...
func _desactivar_bot(bot: BotBase) -> void:
	if not is_instance_valid(bot):
		return
...
func _encontrar_bot_activo_en_equipo(team: int) -> BotBase:
	for bot in bot_pool:
		if not is_instance_valid(bot):
			continue
		if bot.is_dead:
...
## Recibe cualquier pawn (Player o BotBase) y lo mueve al nuevo equipo.
##
...
func _cambiar_equipo_pawn(pawn: Node, nuevo_equipo: int) -> bool:
	...
	if new_team_total >= max_per_team:
		...
		var bot_to_deactivate: BotBase = _encontrar_bot_activo_en_equipo(nuevo_equipo)
...
func _kill_pawn_for_team_change(bot: BotBase) -> void:
	if bot.is_dead:
		return
...
func _check_team_balance(delta: float) -> void:
	if not _match_started or not auto_balance_enabled:
		return
	if not is_instance_valid(GameState) or not GameStateMP.match_active:
		return
...
## Registra un pawn (Player o BotBase) en el sistema y devuelve su player_id.
...
func _sync_all_players_status() -> void:
	for pd in _players_data.values():
		...
		# Determinar estado
		# NOTA: _inactive_bots es Array[BotBase], solo verificamos si pawn es BotBase
		# para evitar errores de TypedArray con el jugador (Player).
		if pawn is BotBase and pawn in _inactive_bots:
			pd.status = PlayerData.Status.INACTIVE
...
## `victim_pawn` es el nodo que murio (Player o BotBase).
