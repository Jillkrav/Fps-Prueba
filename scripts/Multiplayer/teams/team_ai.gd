...
func assign_order(bot: BotBase, order_type: int, target: NodePath = NodePath()) -> void:
...
func get_order_for_bot(bot: BotBase) -> Dictionary:
	if not is_instance_valid(bot):
		return _empty_order()
	if _temp_orders.has(bot):
...
func set_temp_order(bot: BotBase, order_type: int, target_pos: Vector3, duration: float = 10.0, reason: String = "") -> void:
	if not is_instance_valid(bot):
		return
...
func clear_temp_order(bot: BotBase) -> void:
	if _temp_orders.has(bot):
		_temp_orders.erase(bot)


func assign_order_by_role(bot: BotBase) -> void:
	if not is_instance_valid(bot):
		return
		...
		int(Roles.Type.VERSATIL):
			if _needs_defense(team):
				...
				if enemy_core:
					...
					assign_order(bot, OrderType.FREELANCE)
		int(Roles.Type.FRANCOTIRADOR), int(Roles.Type.APOYO):
			if own_core:
				assign_order(bot, OrderType.DEFEND, own_core.get_path())
			else:
				assign_order(bot, OrderType.DEFEND)
...
func assign_orders_for_team(team: int) -> void:
	...
	for npc in npcs:
		if not is_instance_valid(npc) or npc is not BotBase:
			continue
