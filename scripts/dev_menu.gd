## Menu de desarrollo. Se activa con Q.
## Permite: ponerse invisible a los NPC y spawnear NPC configurados manualmente.
extends Control

# ─────────────────────────────────────────
# ESCENAS DE NPC DISPONIBLES
# Para agregar un arma nueva en el futuro: agregar su escena en este diccionario.
# ─────────────────────────────────────────
...
# ─────────────────────────────────────────
# REFERENCIAS DE NODOS (se asignan desde hud.tscn)
# ─────────────────────────────────────────
...
func _ready() -> void:
	# Llenar opciones de los OptionButton
	...
	# Armas disponibles — agregar nuevas aquí + en NPC_SCENES
...
func _on_invisible_pressed() -> void:
	...
	# Agregar o remover al jugador del grupo "invisible_to_npc"
...
func _on_spawn_pressed() -> void:
	...
	# Aplicar atributos elegidos
	npc.relacion    = relacion_id as EnemyBase.Relacion
	npc.experiencia = experiencia_id as EnemyBase.Experiencia

	# Spawnear frente al jugador
	var player: Node3D = get_tree().get_first_node_in_group("player") as Node3D
	if not player:
		push_error("DevMenu: no se encontró al jugador para calcular posición de spawn")
		npc.queue_free()
		return

	var spawn_pos: Vector3 = player.global_transform.origin \
		+ player.global_transform.basis.z * -3.0  # 3 metros adelante del jugador
	spawn_pos.y = player.global_transform.origin.y

	# Añadir al mundo (padre del jugador o a la escena raíz)
